...
const PostData = props => (
	...
	<p>The thing you've got to look out for is that the one-way hash algorithms are well known.  MD5 is a commonly used one, and there are websites out there with hashes and what the passwords are, so if someone did get a hold of data using MD5 hashes they could go on the website and find '2shfjg2850qwer' and see your password is 'cat'.  So what you do is you add a salt to the password.  In my case I take a random string, like 'q93j104i4' and add it to every password, and then hash it.  This salt should be kept secret.  (And for my website, yes, you can go on Github and look at the code on the server side, you'll see a salt in there, but that's not what I'm using on the production server ;) )</p>
	...
	<p>I'll likely be back with one more post this week - stay tuned for that.</p>
